...
from models.TitleALBert import TitleALBert
...
def run(model, sess, train_util, val_util, label_info, saver_path, batch_size=512, keep_prob=0.9,
    ...
    for epoch in range(1, 101):
        ...
        print('ALBERT_pre_train_loss_masked_LM epoch: %d, pre_train_loss: %f' % (epoch, pre_train_loss))

        valid_loss, accuracy = validation_pre_train_masked_LM(model, val_util, batch_size=batch_size)
        print('ALBERT_pre_train_valid_loss: %f, pre_train_valid_accuracy: %f' % (valid_loss, accuracy))
        print()
    ...
    for epoch in range(1, 301):
        ...
        if (epoch) % 5 == 0 or epoch == 1:
            ...
            print("ALBERT_epoch: %d, valid_loss: %f, musin_ndcg: %f, tag_ndcg: %f, score: %f" % (
    ...
    util.dump(best_model_dict, os.path.join(parameters.base_dir, 'ALBERT_plylst_title_best_model_dict.pickle'))
...
model = TitleALBert(
    ...
    hidden_size=parameters.title_model_hidden_size,
...
run(
    ...
    val_util,
    label_info,
    saver_path='./ALBERT_TITLE_VOCA_%d_emb%d_hidden_%d_stack%d_head%d_tags_loss_weight%0.2f_bs_%d_warmup_%d' % (
        parameters.bpe_voca_size, parameters.title_model_embed_size, parameters.title_model_hidden_size,
        parameters.title_model_stack, parameters.title_model_multihead, tags_loss_weight, bs, warmup_steps),
    batch_size=bs,
    keep_prob=0.9,
    tags_loss_weight=tags_loss_weight)
