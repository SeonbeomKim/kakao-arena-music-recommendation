...
from models.OrderlessBertAE2 import OrderlessBertAE
...
def train(model, train_util, iter, batch_size=64, keep_prob=0.9, tags_loss_weight=0.5, artists_loss_weight=0.15):
    ...
    for i in tqdm(range(epoch)):
        ...
        _, _loss, _ranking_loss = sess.run([minimize_func, model.loss, model.ranking_loss],
    ...
    print('ranking_loss:', ranking_loss / epoch)
    return loss / epoch
...
def run(model, sess, train_util, val_util, label_info, saver_path, batch_size=128, keep_prob=0.9,
    ...
    for epoch in range(1, 201):
        ...
        print("SONGS_TAGS_ARTISTS2 epoch: %d, train_loss: %f" % (epoch, train_loss))
        print()
    ...
    util.dump(result, os.path.join(parameters.base_dir, 'songs_tags_artists_result_info2.pickle'))
...
run(
    ...
    val_util,
    label_info,
    saver_path='./SONGS_TAGS_ARTISTS2_emb%d_stack%d_head%d_tags_loss_weight%0.2f_artists_loss_weight%0.2f_bs_%d_warmup_%d' % (
