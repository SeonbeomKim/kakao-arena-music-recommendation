...
from data_loader.songs_tags_artists_util_LM2 import TrainUtil, ValUtil
from evaluate import ArenaEvaluator
from models.OrderlessBertAE_LM2 import OrderlessBertAE
...
def pre_train_masked_LM(model, train_util, iter, batch_size=64, keep_prob=0.9):
    ...
    for i in tqdm(range(epoch)):
        ...
        batch_input_size = pre_train_dataset['input_size'][batch_size * i: batch_size * (i + 1)]
        batch_mask_label = []
        ...
        _, _loss = sess.run([model.minimize_masked_LM_loss, model.masked_LM_loss],
                            {model.input_sequence_indices: batch_input_sequence_indices[:, :max(batch_input_size)],
                             model.boolean_mask: batch_boolean_mask[:, :max(batch_input_size)],
...
def run(model, sess, train_util, val_util, label_info, saver_path, batch_size=128, keep_prob=0.9,
    ...
    for epoch in range(1, 101):
        ...
        print()
        if epoch % 50 == 0:
            model.saver.save(sess, os.path.join(saver_path, "pretrainLM2_%d.ckpt" % epoch))
    ...
    for epoch in range(1, 201):
        ...
        print("SONGS_TAGS_ARTISTS LM2 epoch: %d, train_loss: %f" % (epoch, train_loss))
        print()
...
run(
    ...
    val_util,
    label_info,
    saver_path='./SONGS_TAGS_ARTISTS_LM2_emb%d_stack%d_head%d_tags_loss_weight%0.2f_artists_loss_weight%0.2f_bs_%d_warmup_%d' % (
