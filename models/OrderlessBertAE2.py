...
class OrderlessBertAE:
    def __init__(self, voca_size, embedding_size, is_embedding_scale,
        ...
        with tf.name_scope('ranking_loss'):
            # 틀린걸 잘맞추게 하지 말고, 잘 추천한 노래의 스코어를 높이자

            # song_label이 전부 0인경우 0, 하나라도 있는경우 1
            loss_mask = tf.cast(tf.reduce_sum(song_label, axis=-1) > 0., dtype=tf.float32) # [N]
            highest_correct_score_song_index = tf.argsort(song_label * song_predict, axis=-1,
                                                       direction='DESCENDING')[:, 0]
            highest_wrong_score_song_index = tf.argsort((1 - song_label) * song_predict, axis=-1,
                                                       direction='DESCENDING')[:, 0]  # [N]

            highest_correct_song_embedding = tf.nn.embedding_lookup(  # [N, embedding_size]
                song_embedding_table, highest_correct_score_song_index)
            highest_wrong_song_embedding = tf.nn.embedding_lookup(  # [N, embedding_size]
                song_embedding_table, highest_wrong_score_song_index)

            highest_correct_song_logit = tf.reduce_sum(self.song_cls_embedding * highest_correct_song_embedding,
                                                      axis=-1)  # [N]
            highest_wrong_song_logit = tf.reduce_sum(self.song_cls_embedding * highest_wrong_song_embedding,
                                                    axis=-1)  # [N]

            ranking_loss = -tf.log(tf.nn.sigmoid(highest_correct_song_logit - highest_wrong_song_logit)) # [N]
            self.ranking_loss = tf.reduce_sum(loss_mask * ranking_loss) / (tf.reduce_sum(loss_mask) + 1e-10)
